...
function publicUser(user) {
  return {
    id: String(user.id), name: user.name, username: user.username, email: user.email,
    role: user.role_id ? { id: String(user.role_id), code: user.role_code, name: user.role_name } : null
  };
}
...
async function login({ username, password, metadata }) {
  ...
  if (!user || !passwordMatches) {
    ...
    await createAuditLog({
      userId: user?.id || null, actionCode: 'AUTH_LOGIN_FAILED', entityType: 'AUTH',
      metadata: { ...sanitizeMetadata(metadata), identifierHash: auditIdentifier(normalizedUsername) }
    });
    throw new HttpError(401, 'INVALID_CREDENTIALS', 'Usuário ou senha inválidos.');
  }

  if (!user.is_active || !user.role_id || !user.role_is_active) {
    throw new HttpError(403, 'USER_INACTIVE', 'Acesso não autorizado para este usuário.');
  }
  ...
  return { user: publicUser(user), authVersion: Number(user.auth_version), permissions };
}

async function loadAuthenticatedUser(userId, authVersion) {
  const user = await authRepository.findUserById(userId);
  if (!user || !user.is_active || !user.role_id || !user.role_is_active) return null;
