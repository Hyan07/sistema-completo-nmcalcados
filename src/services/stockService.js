...
async function getSummary() {
  ...
  return {
    ...
    inactiveWithStock: Number(row.inactive_with_stock || 0)
  };
}
...
async function applyStockMovement(input, { connection = null } = {}) {
  const request = {
    ...
    userId: parsePositiveId(input.userId, 'Usuário'),
    typeCode: String(input.typeCode || '').trim().toUpperCase(),
    quantity: parseQuantity(input.quantity),
    reason: normalizeReason(input.reason, { required: false }),
    ...
    saleItemId: input.saleItemId ? parsePositiveId(input.saleItemId, 'Item de venda') : null,
  ...
  try {
    ...
    if (existing) {
      const result = verifyExistingMovement(existing, request);
    ...
    const movementId = await stockRepository.insertMovement({
      skuId: request.skuId,
      ...
      quantityChange: calculated.quantityChange,
      newQuantity: calculated.newQuantity,
      reason: request.reason,
      happenedAt: request.happenedAt
    }, db);
    await stockRepository.updateBalance(request.skuId, calculated.newQuantity, db);
    await createAuditLog({
      userId: request.userId,
      actionCode: 'STOCK_MOVEMENT_CREATED',
      entityType: 'STOCK_MOVEMENT',
      entityId: movementId,
      newData: {
        skuId: String(request.skuId), typeCode: request.typeCode,
        previousQuantity: Number(balance.quantity), quantityChange: calculated.quantityChange,
        newQuantity: calculated.newQuantity, operationKey: request.operationKey
      }
    }, db);
    ...
    if (ownsConnection) {
      ...
      if (error.code === 'ER_DUP_ENTRY') {
        const existing = await stockRepository.findMovementByOperationKey(request.operationKey);
        if (existing) return verifyExistingMovement(existing, request);
      }
    }
    throw error;
  } finally {
...
async function createManualMovement(skuId, input, actor) {
  const normalized = normalizeManualMovement(input);
  return applyStockMovement({ skuId, userId: actor.id, ...normalized });
}

async function countInventory(skuId, input, actor) {
  const id = parsePositiveId(skuId, 'SKU');
  const userId = parsePositiveId(actor.id, 'Usuário');
  const normalized = normalizeInventoryCount(input);
  const connection = await getPool().getConnection();
  try {
    await connection.beginTransaction();
    const existing = await stockRepository.findMovementByOperationKey(normalized.operationKey, connection);
    if (existing) {
      if (String(existing.product_sku_id) !== String(id) || Number(existing.new_quantity) !== normalized.countedQuantity) {
        throw new HttpError(409, 'OPERATION_KEY_REUSED', 'A chave desta contagem já foi utilizada com outros dados.');
      }
      await connection.commit();
      return { movement: existing, duplicate: true, changed: true };
    }

    const sku = await stockRepository.getStockItemById(id, connection);
    if (!sku) throw new HttpError(404, 'SKU_NOT_FOUND', 'SKU não encontrado.');
    await stockRepository.ensureBalance(id, connection);
    const balance = await stockRepository.lockBalance(id, connection);
    ...
    if (committedDuplicate) {
      if (String(committedDuplicate.product_sku_id) !== String(id) || Number(committedDuplicate.new_quantity) !== normalized.countedQuantity) {
        throw new HttpError(409, 'OPERATION_KEY_REUSED', 'A chave desta contagem já foi utilizada com outros dados.');
      }
      await connection.commit();
      return { movement: committedDuplicate, duplicate: true, changed: true };
    }
    const previous = Number(balance.quantity);
    const difference = normalized.countedQuantity - previous;
    if (difference === 0) {
      await createAuditLog({
        userId,
        actionCode: 'STOCK_COUNT_CONFIRMED',
        entityType: 'PRODUCT_SKU',
        entityId: id,
        newData: { quantity: previous, reason: normalized.reason, operationKey: normalized.operationKey }
      }, connection);
      await connection.commit();
      return { movement: null, duplicate: false, changed: false, quantity: previous };
    }

    const result = await applyStockMovement({
      skuId: id,
      userId,
      typeCode: difference > 0 ? 'INVENTORY_GAIN' : 'INVENTORY_LOSS',
