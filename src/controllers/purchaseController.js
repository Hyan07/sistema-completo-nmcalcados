...
const purchaseConfirmationService = require('../services/purchaseConfirmationService');
...
async function confirm(req,res,next){try{res.status(201).json({data:await purchaseConfirmationService.confirmPurchase(req.params.id,req.body,req.user)});}catch(error){next(error);}}
...
module.exports={addItem,cancel,cancelItem,confirm,create,getById,list,lookupSkus,order,receive,update,updateItem};
